...
| R-006 | OAuth 토큰 또는 개인 문서 노출 | Low | Catastrophic | 로컬: AES-GCM 볼트, 로그 REDACTED, 워크스페이스 해싱, 텔레메트리 민감필드 거부, S3 원문 왕복·실패 시 metadata 미생성, CSP/frame/MIME 헤더. 운영 key management·anti-malware·advisory 검토는 없음 | Open (production security evidence required) |
...
| R-013 | 로컬 runtime 검증과 실제 운영 환경의 동작 차이 | Medium | High | standalone/Docker 실제 build, Kubernetes 정적 smoke, fail-closed worker, bounded metric·alert 계약, 실제 local MinIO S3 및 Redis 원자 counter 왕복을 검증. registry push, cluster rollout, managed backup RPO/RTO, production scrape는 명시적으로 차단 | BLOCKED_EXTERNAL |
